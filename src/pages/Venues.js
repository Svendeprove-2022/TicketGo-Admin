...
function Venues() {
    ...
    const venueDataColumns = [
        {title: "Venue Name", dataIndex: 'name'},{title: "Address", dataIndex: 'address'},
        ...
        {title: "Actions", dataIndex:'', render: (_, record) => (
            <Space>
                ...
                <Popconfirm title="Confirm Deletion" 
                            onCancel={null} 
                            onConfirm={() => {onDeleteHandler({variables: { "venueId": record._id}})}}>
        ...
        <PageHeader className="site-page-header" 
                    title="Venues" extra={[
                        <Button key="1" type="primary" 
                                onClick={ () => {navigate("/venues/create")}}> Create</Button>,]
        } />
        <Content>
            <Table rowKey={record => record._id} 
                    columns={venueDataColumns} 
                    loading={loading} 
